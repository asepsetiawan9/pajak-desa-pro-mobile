...
import 'kolektor_leaderboard_screen.dart';
...
class _KadesDashboardState extends State<KadesDashboard> {
  ...
  Future<void> _loadDashboardData() async {
    ...
    final desaIdInt = authProvider.user?.desaId ?? authProvider.user?.desa?.id;
    ...
    await Future.wait([
      ...
      Provider.of<KolektorTargetProvider>(context, listen: false).fetchLeaderboard(currentYear, desaId: desaIdInt),
    ]);
  }
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Executive Section: Monitoring Kinerja Kolektor (Poin 7)
                    _buildMonitoringKolektorSection(context),
                    const SizedBox(height: 24),
  ...
  // ==========================================
  // MONITORING KINERJA KOLEKTOR SECTION (POIN 7)
  // ==========================================
  Widget _buildMonitoringKolektorSection(BuildContext context) {
    final targetProvider = Provider.of<KolektorTargetProvider>(context);
    final leaderboard = targetProvider.leaderboard;
    final isLoading = targetProvider.isLoadingLeaderboard;
    final currentYear = DateTime.now().year;

    final totalKolektor = leaderboard.length;
    final avgPersen = totalKolektor > 0
        ? leaderboard.fold<double>(0.0, (sum, e) => sum + e.persentaseNominal) / totalKolektor
        : 0.0;
    final onTrackCount = leaderboard.where((e) => e.status == 'EXCEEDED' || e.status == 'ON_TRACK').length;
          ...
          Row(
            ...
            children: [
              Row(
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.leaderboard_rounded, color: AppColors.accent, size: 20),
                  ),
                  const SizedBox(width: 10),
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      ...
                      const Text(
                        'Target & Peringkat Penagihan PBB',
                        style: TextStyle(fontSize: 11, color: AppColors.textMuted),
                      ),
                    ],
                  ),
              ...
              IconButton(
                ...
                tooltip: 'Buka Leaderboard Lengkap',
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => KolektorLeaderboardScreen(initialTahun: currentYear),
                    ),
                  );
                },
              ),
          ...
          if (isLoading)
            ...
            Container(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    'Belum ada target kolektor tahun $currentYear',
                    style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: AppColors.textPrimary),
                  ),
                  const SizedBox(height: 2),
                  const Text(
                    'Tetapkan target kolektor melalui aplikasi web.',
                    style: TextStyle(fontSize: 11, color: AppColors.textMuted),
                  ),
                ],
              ),
                ...
                Expanded(
                  child: Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Rata-rata Capaian', style: TextStyle(fontSize: 10, color: AppColors.textMuted)),
                        const SizedBox(height: 2),
                        FittedBox(
                          fit: BoxFit.scaleDown,
                          child: Text(
                            '${avgPersen.toStringAsFixed(1)}%',
                ...
                Expanded(
                  child: Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Kolektor On-Track', style: TextStyle(fontSize: 10, color: AppColors.textMuted)),
                        const SizedBox(height: 2),
                        FittedBox(
                          fit: BoxFit.scaleDown,
                          child: Text(
                            '$onTrackCount dari $totalKolektor',
                            style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: AppColors.success),
                          ),
                        ),
                      ],
                ...
                Expanded(
                  child: Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Top Performer', style: TextStyle(fontSize: 10, color: AppColors.textMuted)),
                        const SizedBox(height: 2),
                        FittedBox(
                          fit: BoxFit.scaleDown,
                          child: Text(
                            leaderboard.first.kolektorName,
            ...
            Column(
              children: top3.asMap().entries.map((entry) {
                ...
                final badgeColor = _getKolektorBadgeColor(item.badge);
                ...
                if (rank == 1) {
                  ...
                  medalBorder = const Color(0xFFD97706);
                } else if (rank == 2) {
                  medalEmoji = '🥈';
                  ...
                  medalEmoji = '🥉';
                  medalBg = const Color(0xFFFFEDD5);
                  medalBorder = const Color(0xFFC2410C);
                }
                            ...
                            Row(
                              children: [
                                Container(
                                  ...
                                  child: Text(medalEmoji, style: const TextStyle(fontSize: 14)),
                                ),
                                const SizedBox(width: 10),
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        '${item.dusunAkses != null ? "Dusun ${item.dusunAkses} · " : ""}${_currency.format(item.realisasiNominal)}',
                                        style: const TextStyle(fontSize: 11, color: AppColors.textMuted),
                                      ),
                                    ],
                                  ),
            ...
            SizedBox(
              ...
              child: OutlinedButton.icon(
                ...
                label: const Text('Buka Semua Peringkat Kolektor', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                style: OutlinedButton.styleFrom(
  ...
  Color _getKolektorBadgeColor(String badge) {
    switch (badge) {
      case 'LEGEND':
