...
/// Model untuk data setoran kecamatan individual
class SetoranItem {
  final int id;
  final int? desaId;
  ...
  factory SetoranItem.fromJson(Map<String, dynamic> json) {
    return SetoranItem(
      ...
      keteranganVerifikasi: json['keterangan_verifikasi']?.toString(),
      tanggalVerifikasi: json['tanggal_verifikasi']?.toString(),
    );
  }
...
/// Provider untuk mengelola data setoran ke kecamatan.
/// Digunakan oleh Admin Kecamatan untuk monitoring & verifikasi setoran.
class SetoranKecamatanProvider extends ChangeNotifier {
  ...
  /// Fetch summary KPI setoran kecamatan
  Future<void> fetchSummary({String? desaId}) async {
    ...
    if (response.success && response.data != null) {
      try {
        ...
        _totalSisaKas = (data['total_sisa_kas_desa'] is num ? data['total_sisa_kas_desa'] : num.tryParse(data['total_sisa_kas_desa']?.toString() ?? '0') ?? 0).toDouble();
  ...
  /// Fetch list setoran
  Future<void> fetchSetoranList({String? desaId}) async {
    _isLoading = true;
    _errorMessage = null;
    notifyListeners();
  ...
  /// Membikin setoran baru ke kecamatan
